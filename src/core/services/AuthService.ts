...
export const verifyToken = async (
  ...
  try {
    ...
    const res = await axios.get("http://192.168.251.121:5000/api/auth/check", {
      headers: {
...
export const signup = async ({ identifier }: { identifier: string }) => {
  ...
  return await fetchHandler('http://192.168.251.121:5000/api/auth/signup', {
    method: 'POST',
...
export const verifySignup = async ({
  ...
  const data = await fetchHandler('http://192.168.251.121:5000/api/auth/verify-signup', {
    method: 'POST',
    body: JSON.stringify({ 
...
export const login = async ({ identifier }: { identifier: string }) => {
  ...
  return await fetchHandler('http://192.168.251.121:5000/api/auth/login', {
    method: 'POST',
    body: JSON.stringify({ identifier: formattedIdentifier }),
  });
};
...
export const verifyLogin = async ({
  ...
  const data = await fetchHandler('http://192.168.251.121:5000/api/auth/verify-login', {
    method: 'POST',
    body: JSON.stringify({ 
      identifier: formattedIdentifier, 
      otp 
...
export const resendOtp = async ({ identifier }: { identifier: string }) => {
  ...
  return await fetchHandler('http://192.168.251.121:5000/api/auth/resend-otp', {
    method: 'POST',
    body: JSON.stringify({ identifier: formattedIdentifier }),
  });
};
