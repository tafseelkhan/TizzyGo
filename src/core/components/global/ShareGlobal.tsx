...
const ProductShare: React.FC<Props> = ({ productId, title }) => {
  ...
  const createShare = async (): Promise<string | null> => {
    try {
      ...
      return buildShareUrl(title, data.share._id);
    } catch (err) {
  ...
  const handleShare = async () => {
    ...
    try {
      await Share.share({
        message: message,
        title: `Check out "${title}" on TizzyGo!`,
      });
    } catch (err) {
