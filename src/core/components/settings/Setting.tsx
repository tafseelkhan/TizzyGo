...
import { SettingsItem as SettingsItemType, ThemeMode, ProfileData } from '../../types/Settings';
...
export const SettingsScreen: React.FC = () => {
  ...
  const { theme, setTheme, isDark, resolvedTheme, loading: themeLoading } = useTheme();
  ...
  const fetchProfile = async () => {
    try {
      ...
      const API_URL = 'http://192.168.42.121:5000'; // Replace with your API URL
      ...
      if (response.ok) {
        ...
        console.error("Profile fetch error:", errorData.message);
  ...
  const Sparkles = () => <MaterialCommunityIcon name="star-four-points" size={24} color="#FFD700" />;
  ...
  const handleLogout = async () => {
    Alert.alert('Logout', 'Are you sure you want to logout?', [
      ...
      { 
        ...
        onPress: async () => {
          try {
            await AsyncStorage.multiRemove(['authToken', 'userData', 'app-theme']);
            navigation.reset({
              index: 0,
      ...
      <View style={styles.themeOptionContent}>
        <View style={styles.themeOptionHeader}>
          ...
          <Text style={[
            styles.themeText,
            { 
              color: isDark ? (theme === mode ? '#6366F1' : '#F1F5F9') : 
                             (theme === mode ? '#6366F1' : '#1E293B')
            }
          ]}>
            {label}
          </Text>
  ...
  const settingsData: SettingsItemType[] = [
    ...
    },
    {
      ...
      rightElement: themeLoading ? (
        ...
        <Icon name="keyboard-arrow-right" size={24} color={isDark ? '#94A3B8' : '#64748B'} />
      )
    },
        ...
        >
          {loading ? (
            <View style={[
              ...
              <ActivityIndicator size="small" color={isDark ? '#94A3B8' : '#6366F1'} />
            </View>
            ...
            <View style={[
              styles.profileIcon,
              { backgroundColor: isDark ? '#334155' : '#F1F5F9', overflow: 'hidden' }
            ]}>
              <LottieView
                ref={animationRef}
                source={require('../animations/lotties/Login icon (1).json')} // Make sure to add your Lottie animation file
        ...
        <View style={[
          ...
          <View style={styles.toggleItem}>
            ...
            <Switch
              ...
              thumbColor={notifications ? '#FFFFFF' : isDark ? '#94A3B8' : '#F1F5F9'}
            />
          </View>
          ...
          <View style={styles.toggleItem}>
            ...
            <Switch
              ...
              trackColor={{ false: isDark ? '#374151' : '#E5E7EB', true: '#10B981' }}
              thumbColor={biometric ? '#FFFFFF' : isDark ? '#94A3B8' : '#F1F5F9'}
            />
          </View>
        </View>
