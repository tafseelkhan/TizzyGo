// components/CommentComponent.tsx - FINAL CRASH FIX VERSION
import React, {
  useState,
  useEffect,
  useRef,
...
// ✅ CRITICAL: Custom comparator for React.memo
const arePropsEqual = (prevProps: any, nextProps: any) => {
  // Only re-render if productId actually changes
  return prevProps.productId === nextProps.productId;
};

// ✅ FIX: Use React.memo with custom comparator
const CommentComponent: React.FC<{ productId: string }> = React.memo(
  ({ productId }) => {
    ...
    // ✅ FIX: Use refs to prevent re-renders
    ...
    console.log(
      `🔵 CommentComponent Render #${renderCountRef.current} - Product ID:`,
      productId,
    );

    // ✅ Only fetch if productId exists and is valid
    ...
    const backendBaseUrl = 'http://172.21.55.121:5000';
      ...
      try {
        ...
        commentsData.forEach((comment: Comment) => {
          ...
          comment.replies.forEach(reply => {
            if (reply.userId && !users[reply.userId]) {
    ...
    // ✅ Debounced functions
    const debouncedFetchCommenters = useRef(
      debounce(async () => {
    ...
    }, [debouncedFetchCommenters, debouncedFetchComments]);
    ...
    useEffect(() => {
      ...
      const initialFetch = async () => {
        if (!productId || hasFetchedInitialRef.current) return;
        hasFetchedInitialRef.current = true;
        await Promise.all([fetchCommenters(), fetchCurrentUserImage()]);
      };

      const timeoutId = setTimeout(initialFetch, 100);
    ...
    useEffect(() => {
      if (showComments && !hasFetchedComments && isMountedRef.current) {
        debouncedFetchComments();
      }
    }, [showComments, hasFetchedComments]);

    // ✅ handleAddComment - STABLE
      ...
      try {
        ...
        if (isMountedRef.current) {
          if (newCommentData.userId && !users[newCommentData.userId]) {
            const [userData] = await fetchUsersBatchAPI([
    ...
    const handleDelete = useCallback(async () => {
      ...
      try {
        ...
        if (isMountedRef.current) {
          setError(
            err.message || `Failed to delete ${showDeleteConfirm?.type}`,
          );
        }
      } finally {
        if (isMountedRef.current) {
    ...
    }, [showDeleteConfirm, currentUserId, debouncedFetchCommenters]);
    ...
    const handleEmojiClick = useCallback(
      (emoji: string, inputType: 'comment' | 'reply') => {
    ...
    const removeMedia = useCallback((type: 'comment' | 'reply') => {
      if (type === 'comment') {
        setNewCommentMedia(null);
        setNewCommentMediaType(null);
      } else {
            ...
            >
              <View style={styles.commentHeader}>
                ...
                >
                  ...
                  />
                  <Text
                    style={[styles.userName, { color: colors.primaryText }]}
                  >
                      ...
                      >
                        ...
                        >
                          <View style={styles.replyHeader}>
                            <TouchableOpacity
                              onPress={() =>
                                handleNavigateToProfile(reply.userId)
                              }
                              style={styles.userInfo}
                            >
                              <Image
                          ...
                          <View style={styles.replyActions}>
                            ...
                            <TouchableOpacity
                              onPress={() =>
                                handleNavigateToReport(reply.userId)
                              }
                              style={[
                                styles.actionButton,
                                isReplyHovered && {
  ...
  arePropsEqual, // ✅ Custom comparator
);

// ✅ IMPORTANT: Add displayName for debugging
CommentComponent.displayName = 'CommentComponent';

// ✅ IMPORTANT: Export with React.memo
