...
// ✅ API function import
import { fetchProduct } from '../../../../api/features/private/productByIdPrivateSlice';
...
const AddToCart: React.FC<AddToCartProps> = ({
  ...
  // ================================
  // ✅ HELPER: Add zeptPayAccountId to product
  // ================================
  const addVendorFields = (product: any) => {
    return {
      ...product,
      zeptPayAccountId:
        product?.zeptPayAccountId ||
        product?.vendorCodeUID ||
        product?.seller?.zeptPayAccountId ||
        'DEFAULT_VENDOR',
      // ✅ Also ensure sellerId is present
      sellerId: product?.sellerId || product?.seller?._id || 'DEFAULT_SELLER',
    };
  };
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      if (!productId) {
        console.warn('⚠️ AddToCart: No productId provided');
      ...
      try {
        console.log('🔄 AddToCart: Fetching product...');
        console.log(`📦 Product ID: ${productId}`);

        // ✅ API call - only productId
        const product = await fetchProduct(productId);

        if (!isMountedRef.current) return;

        console.log('✅ AddToCart: Product fetched successfully');
        console.log(`📋 Product Title: ${product.title}`);
        console.log(`📦 Total Variants: ${product.variants?.length || 0}`);

        // ✅ IMPORTANT: Add zeptPayAccountId to productData
        const productWithVendor = addVendorFields(product);

        console.log('🏷️ zeptPayAccountId:', productWithVendor.zeptPayAccountId);
        console.log('🏷️ sellerId:', productWithVendor.sellerId);

        // ✅ Set product data with vendor field
        setProductData(productWithVendor);
        ...
        if (product.variants && Array.isArray(product.variants)) {
          ...
          if (product.variants.length > 0) {
            ...
            const newVariant = createSelectedVariant(variant, 0);
            setSelectedVariantIndex(0);
            setSelectedVariant(newVariant);
            onVariantSelect(newVariant);
            console.log('📌 Selected first variant by default');
          }
        }
      } catch (error: any) {
        console.error('❌ AddToCart: Error fetching product:', error.message);
        // Fallback: Load variants from CartService
        try {
          ...
          if (isMountedRef.current && fetchedVariants) {
            ...
            if (fetchedVariants.length > 0) {
              const variant = fetchedVariants[0];
              const newVariant = createSelectedVariant(variant, 0);
              setSelectedVariantIndex(0);
              setSelectedVariant(newVariant);
              onVariantSelect(newVariant);
          ...
          console.error('❌ AddToCart: Fallback also failed:', fallbackError);
        }
      } finally {
  ...
  const handleVariantSelect = useCallback(
    (index: number) => {
      triggerVibration();
      if (!variants || !variants.length || index >= variants.length) return;

      const selectedVariantObj = variants[index];
      const newVariant = createSelectedVariant(selectedVariantObj, index);
  ...
  const isVariantSelected = (): boolean => {
    return (
      selectedVariantIndex !== null && selectedVariantIndex < variants.length
    );
  };
  ...
  const handleAddToCart = async () => {
    ...
    try {
      console.log('🛒 Adding to cart with productData:', {
        productId,
      ...
      const success = await CartService.addToCart({
        productId,
        productData, // ✅ Now has zeptPayAccountId
        quantity: 1,
  ...
  const renderVariantModal = () => (
    ...
    >
      {/* ✅ Added SafeAreaView at top level */}
      <SafeAreaView
      ...
      >
        ...
        >
          ...
          {variantsLoading || isProductLoading ? (
  ...
  // ✅ LOADING STATE
  // ================================
  if (isProductLoading && !variants.length) {
    return (
      <View style={[styles.container, styles.loadingContainer]}>
        <ActivityIndicator size={compact ? 'small' : 'large'} color={ORANGE} />
      </View>
    );
  }
