// ProductDetailInfo.tsx (Refactored)
...
const ProductDetailInfo: React.FC = () => {
  ...
  // Use your existing product hook - 1 API call
  const {
    product: rawProduct,
  ...
  // Fetch rating & reviews when product loads
  React.useEffect(() => {
    if (productId && rawProduct) {
  ...
  const onRefresh = () => {
    productRefresh(); // Refresh product
    loadRatingAndReviews(); // Refresh rating & reviews
  };

  // Helper: Filter out variant fields
  ...
  const product = rawProduct ? extractProductWithoutVariants(rawProduct) : null;
  const loading = productLoading || ratingLoading;

  // Dynamic styles (your existing styles - keeping them short here)
  ...
  const getStockStatus = () => {
    ...
    if (product.inStock === false) {
      return { isInStock: false, message: 'Out of Stock' };
    }
  ...
  const getProductFeatures = (): string[] => {
    ...
    booleanFeatures.forEach(({ key, label }) => {
      if (product[key] === true) features.push(label);
    });
    return features;
  };
      ...
      >
        <View style={dynamicStyles.errorContent}>
          ...
          <Text style={dynamicStyles.errorText}>
            {productError || 'Product ID is missing'}
          </Text>
...
// Dynamic styles function (keep your existing styles)
const getDynamicStyles = (isDark: boolean) =>
  StyleSheet.create({
    container: { flex: 1, backgroundColor: isDark ? '#0F172A' : '#F8FAFC' },
    // ... rest of your dynamic styles (copy from original)
    loadingContainer: {
      flex: 1,
    ...
    backButtonText: { color: '#2E8B57', fontSize: 16, fontWeight: '600' },
    productInfoCard: {
      backgroundColor: isDark ? '#1E293B' : '#FFFFFF',
      marginHorizontal: 16,
      marginTop: 16,
    ...
    totalRatings: { fontSize: 13, color: isDark ? '#94A3B8' : '#6B7280' },
    reviewCard: {
      backgroundColor: isDark ? '#1E293B' : '#FFFFFF',
      borderRadius: 12,
      padding: 20,
    ...
    reviewDate: { fontSize: 12, color: isDark ? '#94A3B8' : '#9CA3AF' },
    reviewText: {
      fontSize: 14,
      color: isDark ? '#CBD5E1' : '#4B5563',
      lineHeight: 22,
    ...
    helpfulText: { fontSize: 12, color: isDark ? '#94A3B8' : '#6B7280' },
    ...
    viewAllReviewsText: { fontSize: 15, color: '#2E8B57', fontWeight: '600' },
    noReviewsCard: {
      backgroundColor: isDark ? '#1E293B' : '#FFFFFF',
      marginHorizontal: 16,
      borderRadius: 12,
    ...
    infoText: { fontSize: 14, color: isDark ? '#E2E8F0' : '#374151', flex: 1 },
    infoRow: {
      flexDirection: 'row',
      alignItems: 'center',
      gap: 12,
    ...
    footerText: { fontSize: 10, color: isDark ? '#94A3B8' : '#9CA3AF' },
    modalContainer: {
      flex: 1,
      backgroundColor: isDark ? '#0F172A' : '#F8FAFC',
    },
...
const styles = StyleSheet.create({
  // Static styles (keep all your existing static styles)
  ...
  stockWarningSubtext: { color: '#FECACA', fontSize: 14, marginLeft: 8 },
  stockSuccessText: {
    color: '#FFFFFF',
    fontSize: 18,
    fontWeight: 'bold',
  ...
  stockSuccessSubtext: { color: '#A7F3D0', fontSize: 14, marginLeft: 8 },
  ...
  highlightsGrid: { paddingHorizontal: 16, gap: 10 },
  descriptionSection: { paddingHorizontal: 16, gap: 12, marginTop: 16 },
  descriptionHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    marginBottom: 16,
  ...
  expandButtonText: { fontSize: 13, color: '#0EA5E9', fontWeight: '500' },
  moreSpecsHint: {
    fontSize: 12,
    color: '#6B7280',
    textAlign: 'center',
  ...
  ratingDetails: { alignItems: 'center', gap: 8 },
  reviewsContainer: { paddingHorizontal: 16, gap: 12, marginTop: 16 },
  reviewerAvatar: {
    width: 40,
    height: 40,
    borderRadius: 20,
